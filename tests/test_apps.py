...
class FailingInspectResourcePodman(LifecyclePodman):
    def list_networks(self):
        return [{"Name": "shared-net"}]
...
def test_install_fails_closed_when_existing_resource_cannot_be_inspected():
    from servora.apps import install_app

    p = FailingInspectResourcePodman()
    manifest = {
        "name": "demo",
        "version": "1",
        "services": [{"name": "web", "image": "nginx", "networks": ["shared-net"]}],
    }
